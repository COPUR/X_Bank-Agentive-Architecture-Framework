...
## 1. Containerization, Kong Gateway & EKS Deployment
*   **Implementation Code**: `app/Dockerfile` & `docker-compose.yml`
*   **Mechanism**:
    *   **Kong API Gateway**: Acts as the primary entry point and ingress security layer.
...
*   **Mechanism**:
    ...
    *   Our core services publish and consume messages over distinct event-driven topics:
        ...
        *   `arch-governance-audit`: Used for writing error traces when the cognitive orchestrator halts the GitOps pipeline.

## 3. Localized Stateless Compute Cascades (Mixture of Experts)
*   **Implementation Code**: `vLLM container ports`
*   **Mechanism**: 
    *   **Model**: Llama-3-70B-Instruct running via Hugging Face pipelines (ADR-001).
