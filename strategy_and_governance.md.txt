...
3. Technical Debt Index & Strategic Decision Helper
	•	Implementation Code: app/observability/debt_calculator.py
	•	Mechanism:
	•	debt_calculator.py programmatically scans codebase complexity, coupling, and unmasked logging leaks. It computes a Technical Debt Index (TDI) to automatically generate refactoring backlogs in Jira.
...
4. Software AG Alfabet & CMDB CDC Integration
	•	Implementation Code: app/agents/tools/alfabet_client.py & scripts/cmdb_cdc_listener.py
	•	Mechanism:
	•	app/agents/tools/alfabet_client.py automates REST POST updates to our central portfolio, keeping our Software AG Alfabet logical architectural model synchronized with actual deployments.
	•	cmdb_cdc_listener.py runs as a continuous background listener on EKS, consuming Change Data Capture (CDC) events from our CMDB over the cmdb-cdc-updates Kafka topic. Any physical drift instantly triggers a Semantic Triangle reconciliation.
5. Chapter Developer Performance & Psychological Safety
	•	Implementation Code: app/observability/token_tracker.py & app/agents/agent5_dora.py
